...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    # k = cat([k_ctx, k_noise], dim=-2); RoPE applies only to k_noise (last q_len tokens).
    # k_ctx comes from target model hidden states that already encode position.
    k_ctx = k[..., :-q_len, :]
    k_noise = k[..., -q_len:, :]
    k_noise_embed = (k_noise * cos[..., -q_len:, :]) + (rotate_half(k_noise) * sin[..., -q_len:, :])
    k_embed = torch.cat([k_ctx, k_noise_embed], dim=-2)
...
class DFlashDraftModel(Qwen3PreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        self.block_size = config.dflash_config.get("block_size", 16)
