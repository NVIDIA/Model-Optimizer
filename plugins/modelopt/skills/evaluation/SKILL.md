...
Before reporting scores, read `references/run-validation.md` for NEL timeout/resume behavior and completed-run validation. Complete **Timeout and Output-Limit Accounting** for every task. Parents and evaluators must apply its **Aggregate Model-Output-Fault Policy** per benchmark/run: report category counts, their deduplicated union, the verified unique-successful-response denominator, and the unrounded rate. At ≤2.0%, warn rather than invalidate or retry only when raw responses are preserved and scored incorrect, coverage is complete, and all independent gates pass. Above tolerance, return findings and a recommendation without automatically resubmitting. Never tune token limits merely to pass. For comparisons, apply its **External Baseline Sanity Check**, then hand validated runs to `compare-results`.
