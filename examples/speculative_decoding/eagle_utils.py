...
def get_patched_templated_ring_attn(orig_templated_attn: Callable):
    ...
    def _get_sharded_ttt_msk(i, rank, size, seq_length, ttt_step, dtype):
        ...
        """
        # Get full TTT mask
        attn_bias = _compute_ttt_attention_mask(1, seq_length * size, ttt_step, dtype)
        # Chunk to get current ranks's q rows
        attn_bias = attn_bias.chunk(size, dim=2)[rank]
        # Split cols into seq_length blocks
        attn_bias = attn_bias.split(seq_length, dim=3)
        # Get interleaved col blocks for current rank
        attn_bias = attn_bias[(rank - i) % size :: size]
        return torch.cat(attn_bias, dim=3)
