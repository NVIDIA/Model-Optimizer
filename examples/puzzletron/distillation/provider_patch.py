...
def _has_fully_pruned_slot(block_configs: list[Any] | None) -> bool:
    ...
    for bc in block_configs:
        ...
        attn_no_op = (
            (
                getattr(attn, "no_op", False)
                if not isinstance(attn, dict)
                else attn.get("no_op", False)
            )
            if attn is not None
            else False
        )
...
def set_provider_block_configs(
    provider: Any,
    block_configs: list[dict | Any] | None,
) -> None:
...
def apply_patch() -> None:
    ...
    logger.info(
        "apply_patch: patched ModelProviderMixin.provide for heterogeneous AnyModel support"
    )
...
def _patched_provide(self: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    logger.info(
        "%s.provide(): activating mbridge_patcher (block_configs=%d layers, apply_no_ops=%s)",
        type(self).__name__,
        len(block_configs),
        apply_no_ops,
    )
...
def apply_distillation_patch() -> None:
    ...
    def _patched_distillation_provide(
        ...
        def _wrapped_super_provide(
            self_inner: Any, pre: Any = None, post: Any = None, vp: Any = None
        ) -> Any:
            with mbridge_patcher(
...
def set_student_block_configs(
    ...
    student_block_configs: list[dict | Any] | None,
) -> None:
