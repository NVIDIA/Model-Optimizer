...
class BaseRunner:
    ...
    def process_metrics_final(self, text_outputs, failed_request_ids=None):
        self._ensure_metric_records()
        failed_request_ids = set(failed_request_ids or ())
        records = [r for r in self.metric_records if r["request_id"] not in failed_request_ids]
        for metric in self.metrics:
            metric.process_final(text_outputs, records, failed_request_ids=failed_request_ids)
