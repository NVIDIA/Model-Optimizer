...
def build_metrics(args, tokenizer, dataset_name, dataset, metrics_module):
    ...
    elif dataset_name == "speed" and getattr(args, "agentic", False):
        metrics_list.insert(0, metrics_module.Agentic(requests=dataset.data))
...
async def gather_limited(
    ...
    async def worker_loop():
        ...
        while True:
            ...
            if result_ready and on_result is not None and not abort_event.is_set():
                try:
    ...
    try:
        await asyncio.wait({join_task, abort_task}, return_when=asyncio.FIRST_COMPLETED)
        ...
        for task in (join_task, abort_task):
            if not task.done():
                task.cancel()
        await asyncio.gather(join_task, abort_task, return_exceptions=True)
        for task in tasks:
            if not task.done():
                task.cancel()
