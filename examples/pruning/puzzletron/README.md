...
python -m pip install -r examples/pruning/puzzletron/requirements.txt
```
...
3. Run the puzzletron pipeline.
   ...
   ```bash
   torchrun --nproc_per_node 2 examples/pruning/puzzletron/main.py --config examples/pruning/puzzletron/configs/llama-3_1-8B_pruneffn_memory/llama-3_1-8B_pruneffn_memory.yaml 2>&1 | tee ./log.txt | grep "Puzzletron Progress"
   ```
...
```bash
torchrun --nproc_per_node 2 examples/pruning/puzzletron/main.py \
   --config examples/pruning/puzzletron/configs/llama-3_1-8B_pruneffn_memory/llama-3_1-8B_pruneffn_memory.yaml \
...
For distillation results on Puzzletron-compressed models, see [DISTILLATION_RESULTS.md](./DISTILLATION_RESULTS.md).
...
```bash
torchrun --nproc_per_node 2 examples/pruning/puzzletron/main.py \
   --config examples/pruning/puzzletron/configs/llama-3_1-8B_pruneffn_runtime/llama-3_1-8B_pruneffn_runtime.yaml 2>&1 | tee ./log.txt | grep "Puzzletron Progress"
```
