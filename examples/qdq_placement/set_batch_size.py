...
def set_batch_size(model_path: str, batch_size: int, output_path: str) -> None:
    ...
    print(f"Modified input shape: {[d.dim_param or d.dim_value for d in input_tensor.type.tensor_type.shape.dim]}")
...
Examples:
  ...
  python set_batch_size.py resnet50_Opset17.onnx --batch-size 1 --output resnet50.bs1.onnx
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Batch size to set (default: 128)"
    )
    parser.add_argument(
        "--output", "-o",
        help="Path to save modified model (default: <model>_bs<batch_size>.onnx)"
    )
