...
6.  Configure parallelism on both providers.
7.  Build the training ``ConfigContainer`` via Bridge's ``_pretrain_common()`` skeleton
    and set ``cfg.model = student_provider``.
8.  Wrap student + teacher providers into a ``DistillationProvider`` via
    ``convert_to_distillation_provider()``.
9.  Attach per-layer block_configs to student and teacher.
10. Apply YAML overrides (``--config-file``) and Hydra-style CLI overrides.
11. Call Bridge's ``distill()`` to run training.
...
------------------------
1. Defaults from ``_pretrain_common()`` (minimal training skeleton).
2. YAML file from ``--config-file`` (if provided).
...
DEFAULT_CONFIG_FILE: Path = SCRIPT_DIR / "kd.yaml"
...
def _build_config_container(student_provider, student_load_path: str) -> ConfigContainer:
    """Build a minimal training ConfigContainer around the student provider.

    Uses Bridge's ``_pretrain_common()`` as the skeleton, then:
    - Sets ``cfg.model`` to the student provider.
    - Configures the tokenizer to load from the student checkpoint path.
    - Sets ``cfg.dataset.seq_length`` from the provider's ``seq_length`` attribute.

    The caller is responsible for applying YAML/CLI overrides on top of this.
    """
    cfg = _pretrain_common()

    # Student provider is the model; DistillationProvider wraps it later.
    cfg.model = student_provider

    # Tokenizer: point at the student checkpoint so HF tokenizer is loaded.
    # Users can override to NullTokenizer (for pre-tokenized binary datasets) via YAML:
    #   tokenizer:
    #     tokenizer_type: NullTokenizer
    #     vocab_size: 131072
    cfg.tokenizer.tokenizer_type = "HuggingFaceTokenizer"
    cfg.tokenizer.tokenizer_model = student_load_path

    # Dataset seq_length defaults to the provider's seq_length if set.
    provider_seq_len = getattr(student_provider, "seq_length", None)
    if provider_seq_len:
...
def main(args: argparse.Namespace) -> None:  # noqa: C901 (complexity OK for an orchestration fn)
    ...
    # ------------------------------------------------------------------
    # Step 7: Build ConfigContainer around the student provider
    # ------------------------------------------------------------------
    logger.info("Step 7: Building ConfigContainer")
    cfg = _build_config_container(student_provider, student_path)

    # ------------------------------------------------------------------
    # Step 8: Create DistillationProvider
    ...
    #   After this, cfg.model IS the DistillationProvider.
    # ------------------------------------------------------------------
    logger.info("Step 8: Creating DistillationProvider")
    kd_config = ModelOptDistillConfig()
    cfg.model = convert_to_distillation_provider(cfg.model, teacher_provider, kd_config)
    logger.info("  DistillationProvider created (student=%s, teacher=%s)",
                cfg.model._super_class.__name__, type(teacher_provider).__name__)

    # ------------------------------------------------------------------
    # Step 9: Attach block_configs to student and teacher
    #
    ...
    # ------------------------------------------------------------------
    logger.info("Step 9: Attaching block_configs")
    set_student_block_configs(cfg.model, student_block_configs)
    set_provider_block_configs(cfg.model.teacher, teacher_block_configs)

    # ------------------------------------------------------------------
    # Step 10: Apply YAML and CLI configuration overrides
    # ------------------------------------------------------------------
    logger.info("Step 10: Applying configuration overrides")

    if get_rank_safe() == 0:
        logger.info("--- Base configuration ---")
        cfg.print_yaml()

    merged_cfg, excluded_fields = create_omegaconf_dict_config(cfg)

    # YAML file overrides (--config-file)
    ...
    # Hydra-style CLI overrides (highest priority)
    if args.overrides:
    ...
    # ------------------------------------------------------------------
    logger.info("Step 11: Starting distillation")
    distill(config=cfg)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"Defaults to kd.yaml in the script directory (if it exists)."
        ),
    )
    parser.add_argument(
        "--trust-remote-code",
