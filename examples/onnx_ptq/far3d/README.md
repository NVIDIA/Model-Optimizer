...
git apply --unidiff-zero /path/to/Model-Optimizer/examples/onnx_ptq/far3d/far3d-runtime.patch
cd ../..
```

The second patch adapts FAR3D's legacy FlashAttention import to the FlashAttention 2 API included in the PyTorch container and removes an unused remote-file dependency. The reference configuration continues to use MMCV `MultiheadAttention`.
...
docker run --rm -it --gpus=all --shm-size=80G \
  ...
  -v /path/to/av2_sensor:/workspace/far3d-trt/data/av2 \
  far3d-modelopt
```

Use the container's Python environment for data preparation, export, quantization, and evaluation:
...
Run Model Optimizer in the same Python environment:
