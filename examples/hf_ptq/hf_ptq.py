...
def auto_quantize(
    ...
    if auto_quantize_method in {"gradient", "parent_recon"}:
        ...
        raise ValueError(
            ...
            "Must be 'gradient', 'parent_recon', or 'kl_div'"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="gradient",
        choices=["gradient", "parent_recon", "kl_div"],
        help=(
            "Method for auto_quantize sensitivity analysis. 'gradient' uses gradient-based method "
            "(requires labels in dataset). 'parent_recon' uses normalized reconstruction error at "
            "parent attention/MLP outputs. 'kl_div' uses KL divergence between original and "
    ...
    parser.add_argument(
        ...
        help=(
            "Boundary used for AutoQuantize sensitivity scoring. 'parent' measures attention and "
            "MoE projection perturbations at their parent attention/MLP outputs without grouping "
            "the projection recipe decisions. Defaults to 'parent' for parent_recon and 'local' "
            "for other methods."
        ),
    )
    parser.add_argument(
