...
def _force_attn_implementation(model, attn_implementation: str) -> None:
    """Re-apply ``--attn_implementation`` to the model config and every nested sub-config.

    Remote modeling code may overwrite the requested backend in ``__init__`` (Kimi-K3 forces
    ``flash_attention_2``). Export runs a trace forward, so an unavailable backend fails
    there rather than at load. Only invoked when the caller passed the flag explicitly.
    """
    ...
    while pending:
        cfg = pending.pop()
        if not isinstance(cfg, PretrainedConfig) or id(cfg) in seen:
            continue
        seen.add(id(cfg))
        ...
        if current is not None and current != attn_implementation:
            try:
                ...
                warnings.warn(f"Could not set attn_implementation on {type(cfg).__name__}: {e}")
        # Any attribute that is itself a config, rather than a fixed name list: remote-code
        # models nest them under names transformers does not declare in ``sub_configs``.
        pending.extend(vars(cfg).values())
...
def _pin_offloaded_modules(model, suffixes: tuple[str, ...]) -> int:
    ...
    for name, module in model.named_modules():
        if not name.endswith(suffixes):
            continue
        # Not getattr(module, "_hf_hook"): accelerate wraps it in a SequentialHook once a
        # second hook is appended, and a raw getattr silently misses that.
...
def get_model(
    ...
    if _disk_offload and pin_offloaded_modules:
        n = _pin_offloaded_modules(model, tuple(pin_offloaded_modules))
        print(f"Pinned {n} offloaded module(s) that are read outside their own forward.")
