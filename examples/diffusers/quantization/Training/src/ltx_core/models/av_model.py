...
from ltx_core.models.model import (
    AdaLayerNormSingle,
    CrossAttention,
    FeedForward,
    LTXVModel,
...
from ltxv_trainer.ltxv_utils import unpack_latents


def rms_norm(x: torch.Tensor) -> torch.Tensor:
    return F.rms_norm(x, normalized_shape=(x.shape[-1],), eps=1e-6)
...
class BasicAVTransformerBlock(nn.Module):
    def __init__(
        ...
        self.audio_scale_shift_table = nn.Parameter(torch.empty(6, a_dim, device=device, dtype=dtype))

        self.scale_shift_table_a2v_ca_audio = nn.Parameter(torch.empty(5, a_dim, device=device, dtype=dtype))
    ...
    def forward(
        ...
        run_ax = transformer_options.get("run_ax", False)
        ...
        if run_vx:
            vshift_msa, vscale_msa, vgate_msa, vshift_mlp, vscale_mlp, vgate_mlp = self.get_ada_values(
                self.scale_shift_table, vx.shape[0], v_timestep
            )

            norm_vx = rms_norm(vx) * (1 + vscale_msa) + vshift_msa
            vx = vx + self.attn1(norm_vx, pe=v_pe, transformer_options=transformer_options) * vgate_msa
            vx = vx + self.attn2(
                rms_norm(vx),
                context=v_context,
                mask=attention_mask,
        ...
        if run_ax:
            ashift_msa, ascale_msa, agate_msa, ashift_mlp, ascale_mlp, agate_mlp = self.get_ada_values(
                self.audio_scale_shift_table, ax.shape[0], a_timestep
            )
...
class LTXAVModel(LTXVModel):
    ...
    def _init_output_components(self, device, dtype):
        ...
        self.audio_scale_shift_table = nn.Parameter(torch.empty(2, self.audio_inner_dim, dtype=dtype, device=device))
        ...
        self.audio_proj_out = nn.Linear(self.audio_inner_dim, self.audio_out_channels, dtype=dtype, device=device)
        self.a_patchifier = AudioPatchifier(1, start_end=True)

    def _separate_audio_and_video_latents(self, x, audio_length):
        ...
        ax = ax.reshape(ax.shape[0], self.num_audio_channels, audio_length, self.audio_frequency_bins)

        return vx, ax

    def _recombine_audio_and_video_latents(self, vx, ax, target_shape=None):
        """Recombine audio and video latents for output."""
        # Make sure that audio and video latents are on the same device and dtype.
        if ax.device != vx.device or ax.dtype != vx.dtype:
            logging.warning(f"Audio and video latents are on different devices or dtypes.")
        ...
        repetitions = math.ceil(ax.shape[-1] / divisor) if target_shape is None else target_shape[1] - vx.shape[1]
    ...
    def _process_input(self, x, keyframe_idxs, denoise_mask, **kwargs):
        ...
        input_is_packed = x.ndim == 3

        if input_is_packed:
            latent_shape = kwargs.get("latent_shape")
            if latent_shape is None:
                raise ValueError("latent_shape must be provided in kwargs when input is packed [B, seq_len, C]")
            num_frames, height, width = latent_shape
            x = unpack_latents(x, num_frames, height, width)

            if denoise_mask is not None:
                denoise_mask = unpack_latents(
                    denoise_mask.unsqueeze(-1).float(),
                    num_frames,
                    height,
                    width,
                ).squeeze(1)
        ...
        vx, ax = self._separate_audio_and_video_latents(x, audio_length)
        [vx, v_pixel_coords, additional_args] = super()._process_input(vx, keyframe_idxs, denoise_mask, **kwargs)

        ax, a_latent_coords = self.a_patchifier.patchify(ax)
        ax = self.audio_patchify_proj(ax)

        # Store audio_length and number of audio tokens for use in _prepare_timestep and _process_output
        num_audio_tokens = ax.shape[1]  # Sequence length dimension
        additional_args.update({
            "av_orig_shape": list(x.shape),
    ...
    def _prepare_timestep(self, timestep, batch_size, hidden_dtype, **kwargs):
        ...
        v_embedded_timestep = v_embedded_timestep.view(batch_size, -1, v_embedded_timestep.shape[-1])

        # Check if we're in video-only mode (no audio tokens)
        num_audio_tokens = kwargs.get("num_audio_tokens", None)
        is_video_only = num_audio_tokens == 0

        # Prepare audio timestep
        a_timestep = kwargs.get("a_timestep")

        if is_video_only:
            # Video-only mode: create empty audio timestep embeddings
            # Shape: [batch_size, 0, hidden_dim]
            device = v_timestep.device
            a_timestep = torch.empty(batch_size, 0, v_timestep.shape[-1], dtype=hidden_dtype, device=device)
            a_embedded_timestep = torch.empty(batch_size, 0, v_embedded_timestep.shape[-1], dtype=hidden_dtype, device=device)
            # Create empty cross-attention timesteps (4 elements expected by _process_transformer_blocks)
            empty_ca_timestep = torch.empty(batch_size, 0, v_timestep.shape[-1], dtype=hidden_dtype, device=device)
            cross_av_timestep_ss = [empty_ca_timestep, empty_ca_timestep, empty_ca_timestep, empty_ca_timestep]
        elif a_timestep is not None:
            ...
            cross_av_timestep_ss = list([t.view(batch_size, -1, t.shape[-1]) for t in cross_av_timestep_ss])
        else:
            a_timestep = timestep
            a_embedded_timestep = kwargs.get("embedded_timestep")
            cross_av_timestep_ss = []

        return [v_timestep, a_timestep, cross_av_timestep_ss], [v_embedded_timestep, a_embedded_timestep]

    def _prepare_context(self, context, batch_size, x, attention_mask=None):
        vx = x[0]
        ax = x[1]
        if context.shape[-1] == self.caption_channels * 2:
            v_context, a_context = torch.split(context, int(context.shape[-1] / 2), len(context.shape) - 1)
        else:
            v_context = a_context = context

        v_context, attention_mask = super()._prepare_context(v_context, batch_size, vx, attention_mask)
    ...
    def _prepare_positional_embeddings(self, pixel_coords, frame_rate, x_dtype):
        ...
        max_pos = max(self.positional_embedding_max_pos[0], self.audio_positional_embedding_max_pos[0])
        ...
        for i, block in enumerate(self.transformer_blocks):
            if ("double_block", i) in blocks_replace:
                ...
                if self._enable_gradient_checkpointing and self.training:
                    # Create a wrapper function for gradient checkpointing
                    def create_forward_function(block_module):
                        def forward_function(
                            ...
                            return block_module(
                                ...
                                v_pe=v_pos_emb,
                                a_pe=a_pos_emb,
                                v_cross_pe=v_cross_pos_emb,
                                a_cross_pe=a_cross_pos_emb,
                                v_cross_scale_shift_timestep=v_cross_ss_ts,
                                a_cross_scale_shift_timestep=a_cross_ss_ts,
                                v_cross_gate_timestep=v_cross_gate_ts,
                                a_cross_gate_timestep=a_cross_gate_ts,
                                transformer_options=trans_opts,
                            )
                        return forward_function

                    vx, ax = torch.utils.checkpoint.checkpoint(
                        create_forward_function(block),
                        (vx, ax),
                        v_context,
                        a_context,
                        attention_mask,
                        ...
                        v_pe,
                        a_pe,
                        av_cross_video_freq_cis,
                        av_cross_audio_freq_cis,
                        av_ca_video_scale_shift_timestep,
                        av_ca_audio_scale_shift_timestep,
                        av_ca_a2v_gate_noise_timestep,
                        av_ca_v2a_gate_noise_timestep,
    ...
    def _process_output(self, x, embedded_timestep, keyframe_idxs, **kwargs):
        ...
        input_is_packed = kwargs.get("input_is_packed", False)
        if input_is_packed:
            kwargs_copy = kwargs.copy()
            kwargs_copy["input_is_packed"] = False
            vx = super()._process_output(vx, v_embedded_timestep, keyframe_idxs, **kwargs_copy)
        else:
        ...
        if is_video_only:
            # Video-only mode: skip audio processing and return video-only output
            if input_is_packed:
                from ltxv_trainer.ltxv_utils import pack_latents
                result = pack_latents(vx)
                return result
            else:
                # Return vx directly (already processed by parent's _process_output)
                return vx

        # Process audio output (only when audio is present)
        a_scale_shift_values = (
            self.audio_scale_shift_table[None, None].to(a_embedded_timestep.dtype) + a_embedded_timestep[:, :, None]
        )
        ...
        result = self._recombine_audio_and_video_latents(vx, ax, original_shape)

        if input_is_packed:
            from ltxv_trainer.ltxv_utils import pack_latents

            result = pack_latents(result)
...
if __name__ == "__main__":
    ...
    model_params = torch.load("in.pt", weights_only=False)
    exp = torch.load("out.pt")

    model_checkpoint = "/models/comfyui_models/checkpoints/ltx-av-step-1812100.safetensors"
    with st.safe_open(model_checkpoint, framework="pt", device="cpu") as f:
        sd = {}
        metadata = f.metadata()
        for k in f.keys():
            if k.startswith("model.diffusion_model."):
                if "audio_embeddings_connector" in k or "video_embeddings_connector" in k:
                    continue
                tensor = f.get_tensor(k)
                sd[k.removeprefix("model.diffusion_model.")] = tensor

    config = json.loads(metadata["config"])["transformer"]
    with torch.device("meta"):
        model = LTXAVModel(**config).requires_grad_(False)
    ...
    print(result.shape)
    err = (result - exp).abs()
    print(f"Exp min: {exp.min()}, max: {exp.max()}, abs mean: {exp.abs().mean()}")
    print(f"Result min: {result.min()}, max: {result.max()}, abs mean: {result.abs().mean()}")
    print(f"Err min: {err.min()}, max: {err.max()}, mean: {err.mean()}")
