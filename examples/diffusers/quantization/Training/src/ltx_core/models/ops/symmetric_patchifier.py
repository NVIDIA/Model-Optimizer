...
class Patchifier(ABC):
    ...
    def get_latent_coords(
        self, latent_num_frames: int, latent_height: int, latent_width: int, batch_size: int, device: torch.device
    ) -> torch.Tensor:
        """
        ...
        delta = torch.tensor(
            ...
            dtype=latent_sample_coords_start.dtype,
        )[:, None, None, None]
...
class AudioPatchifier(Patchifier):
    ...
    def copy_with_shift(self, shift: int) -> "AudioPatchifier":
    ...
    def _get_audio_latent_time_in_sec(
        self, start_latent: int, end_latent: int, dtype: torch.dtype, device: torch.device
    ) -> torch.Tensor:
    ...
    def patchify(self, audio_latents: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        audio_latents_start_timings = self._get_audio_latent_time_in_sec(
            self.shift, t + self.shift, torch.float32, audio_latents.device
        )
        audio_latents_start_timings = audio_latents_start_timings.unsqueeze(0).expand(b, -1).unsqueeze(1)

        if self.start_end:
            audio_latents_end_timings = self._get_audio_latent_time_in_sec(
                self.shift + 1, t + self.shift + 1, torch.float32, audio_latents.device
            )
