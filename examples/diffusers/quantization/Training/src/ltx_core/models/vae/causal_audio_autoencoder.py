...
class StringConvertibleEnum(Enum):
    ...
    @classmethod
    def str_to_enum(cls, value: str | None | StringConvertibleEnum) -> StringConvertibleEnum:
        """
...
def normalize(in_channels: int, *, num_groups: int = 32, normtype: str = "group") -> nn.Module:
    if normtype == "group":
...
def make_attn(in_channels: int, attn_type: str = "vanilla", norm_type: str = "group") -> nn.Module:
...
class Decoder(nn.Module):
    ...
    def forward(
        self, latent_features: torch.Tensor, target_shape: tuple[int, int, int, int] | None = None
    ) -> torch.Tensor:
        """
...
class CausalAudioAutoencoder(nn.Module):
    ...
    def decode(self, x: torch.Tensor, target_shape: tuple[int, int, int, int] | None = None) -> torch.Tensor:
