...
def apply_loras(
    model_sd: StateDict, lora_sd_and_strengths: list[LoraStateDictWithStrength], dtype: torch.dtype
) -> StateDict:
    sd = {}
    size = 0
    ...
    for key, weight in model_sd.sd.items():
        ...
        deltas = _prepare_deltas(lora_sd_and_strengths, key, dtype, device)
        if deltas is None:
            deltas = weight.clone().to(dtype=dtype, device=device)
        ...
        sd[key] = deltas
        size += deltas.nbytes
    return StateDict(sd, device, size, dtype)
