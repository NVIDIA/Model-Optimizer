...
def decode_and_preprocess_image(image_path: str, dtype: torch.dtype, device: torch.device, crf=29) -> torch.Tensor:
    image = Image.open(image_path)
    np_array = np.array(image)[..., :3]
    processed_image = preprocess(np_array, crf)
    return torch.tensor(processed_image, dtype=dtype, device=device)
...
def preprocess(image: np.array, crf=29):
    if crf == 0:
        return image
