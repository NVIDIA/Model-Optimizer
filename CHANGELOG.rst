...
- Fix vLLM 0.24+ support, verified on 0.24 (as shipped in ``nemo:26.08``) and 0.26: ``FusedMoE`` became a factory function and the fused expert weights moved onto a ``RoutedExperts`` submodule, so registering the old class put a plain function in ``QuantModuleRegistry`` and every later lookup raised ``TypeError: issubclass() arg 2 must be a class``, breaking unrelated quantization paths. The vLLM plugin now registers whichever fused-MoE module class the installed release provides. Note that the registry key moves ``vllm_FusedMoE`` to ``vllm_RoutedExperts`` and quantizer module paths gain ``.routed_experts``, so a ``modelopt_state`` saved under an older vLLM does not restore onto 0.24+ as-is.
- Fix ``examples/vllm_serve`` dropping shared-expert quantizer state on reload: the HF-to-vLLM key mapping skipped gate/up merging for any key containing ``experts``, which also matched ``shared_experts``. vLLM merges the shared expert's ``gate_proj``/``up_proj`` into ``gate_up_proj``, so those keys resolved to no module and the shared experts were served uncalibrated.
