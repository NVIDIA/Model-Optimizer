...
- Export an FSDP2-sharded model to a HuggingFace checkpoint without gathering it: ``export_hf_checkpoint`` now detects an FSDP2 model under ``torch.distributed`` and routes it to ``distributed_save_hf_checkpoint``, which moves each tensor to a single owner rank and has every rank write its own shards in parallel, instead of reconstructing the full state dict in host RAM. MoE experts additionally stay fused and sharded through the quantizer fold, so each rank materializes only its local experts rather than all of them. Measured on 16 GPUs / 4 nodes: Llama-3.1-405B-Instruct FP8 exports in 149s against 307s for the gather path, with peak host RSS of 65 GB against 430 GB (the state-dict step alone falls from 99.7s to 1.6s) and a tensor-for-tensor identical checkpoint. Qwen3-235B-A22B FP8, which the gather path cannot export at all -- it exhausts GPU memory materializing every expert on every rank during the quantizer fold, before any writer runs -- exports in 8 minutes.
