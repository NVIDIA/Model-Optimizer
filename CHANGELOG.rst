...
- Extend Claude Code agent skills for PTQ, deployment, evaluation, monitoring, debugging, and baseline-vs-quantized result comparison. Adds evaluation task references for additional benchmarks, stronger PTQ checkpoint validation gates, session-scoped workspace/job tracking, and a debugging playbooks skill for recurring framework/runtime failures.
