...
- Fix tied-weight deduplication during HF checkpoint export, which previously keyed on tensor ``data_ptr()``. Address identity misfires in several ways -- a freed address recycled by the allocator can falsely alias two unrelated weights, and the FSDP full-state-dict gather (and offload) materializes tied weights at distinct addresses so a genuine tie is missed and both copies are written. Ties are now detected by shared object identity before packing and recorded as **names** (the model's ``_tied_weights_keys`` / ``tie_word_embeddings`` only choose which side is canonical); the duplicate is dropped by name in ``postprocess_state_dict`` -- correct on 1 GPU and under multi-GPU FSDP alike (e.g. a 4-GPU MiniMax export). Both the per-module dense and fused-MoE ``data_ptr``-keyed dedup caches are removed: every tied weight is now packed independently and the duplicate is dropped by name. A ``(device, storage_ptr, storage_size)`` pass is retained only as a backstop for undeclared same-storage shares (which ``safetensors.save_file`` would otherwise refuse to serialize); it fires only for unquantized/unpacked shared weights, since quantized ties pack to distinct storage and are collapsed by name.
