...
- Skip-softmax sparse-attention HF export now emits the checkpoint schema consumed by the TensorRT-LLM VisualGen runtime for diffusion transformers: calibration coefficients under ``threshold_scale_factor.coefficients``, a scalar ``target_sparsity``, and a normalized ``disabled_until_timestep`` cutoff (renamed from the integer ``initial_disabled_steps``; the deprecated name is no longer emitted). The LLM (prefill/decode) export schema is unchanged.
