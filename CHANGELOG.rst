...
- Add opt-in ``torch.compile`` execution for the per-expert weight-quantizer loop in Transformer Engine ``GroupedLinear`` QAT/QAD. Set ``MODELOPT_TEGROUPED_COMPILE_WEIGHT_LOOP=1`` before quantized-module conversion to compile only the quantizer loop with Inductor while leaving the surrounding grouped GEMM eager; the default path remains unchanged.
