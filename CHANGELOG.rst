...
- Fix ``python -m modelopt.onnx.quantization`` rejecting ``--trust_calibration_data`` when combined with ``--calibration_data_path`` or ``--calibration_cache_path``. The flag was in the same mutually exclusive group as those paths, so the pickle opt-in that the error message points users to was unreachable from the CLI.
