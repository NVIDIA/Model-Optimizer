...
- A recipe can now **delegate its whole body to another recipe** with a top-level ``$import``, overriding only ``metadata``. ``metadata.recipe_type`` became optional along with it: a recipe states its kind with a ``# modelopt-schema:`` comment, with ``metadata.recipe_type``, or by delegating to a recipe that does, and only a recipe that another file imports has to carry the schema comment. Whatever a recipe does state must be true: a schema comment and a ``recipe_type`` must agree, and so must a recipe and the recipe it delegates to. ``modelopt_recipes/models/`` uses this to give every published checkpoint an entry at its own hub path: a thin alias when a portable recipe reproduces the release unchanged, a full body only when it deviates.
- Add ``modelopt_recipes/published_checkpoints.md``: every model in NVIDIA's `Inference Optimized Checkpoints <https://huggingface.co/collections/nvidia/inference-optimized-checkpoints-with-model-optimizer>`_ collection that NVIDIA publishes, paired with the recipe that reproduces its quantization layout, so a released checkpoint can be re-created from its source weights. Partner-published checkpoints in the collection are listed but not given recipes. Each pairing is checked against the release's own per-module quantization map by ``tools/recipe_backfill/``.
- Add four KV-unquantized variants of the general PTQ body schemes -- ``nvfp4_default-kv_fp16``, ``fp8_default-kv_fp16``, ``nvfp4_mlp_only-kv_fp16`` and ``nvfp4_experts_only-kv_fp16`` -- for the released checkpoints that ship with ``kv_cache_quant_algo: null``, plus checkpoint recipes for the NVIDIA-source models (NVIDIA-Nemotron-3-Nano-30B-A3B, NVIDIA-Nemotron-3-Super-120B-A12B FP8 and Nemotron-3-Nano-Omni-30B-A3B-Reasoning). Recipes for the remaining source-model orgs follow in separate changes.
