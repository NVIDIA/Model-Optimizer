...
- Quantize HF transformers 5.x fused-MoE experts (``@use_experts_implementation``) directly in the fast ``grouped_mm`` backend instead of forcing the slow eager (per-expert ``F.linear``) forward. ``_QuantFusedExperts`` now also intercepts ``transformers.integrations.moe._grouped_linear``, reusing the same per-expert weight quantizers and shared input quantizers, so ``grouped_mm`` calibration produces a checkpoint identical to the eager path while scaling to large expert counts. ``local_hessian`` captures per-expert activations natively in grouped mode (splitting the expert-sorted batch by offsets). Unsupported backends (``batched_mm``, ``sonicmoe``, ``kernels``) are now redirected to ``grouped_mm`` rather than eager (``force_eager_experts_impl_on_the_fly`` → ``normalize_experts_impl_on_the_fly``). The transposed-weight layout (e.g. GptOss) keeps its dedicated wrapper.
