...
- Support the **FlashInfer** attention backend for skip-softmax calibration (in addition to FlashAttention). A backend-agnostic ``_SparseCalibrationMixin`` shares the per-request measurement; ``ModelOptSparseFlashInferImpl`` reads FlashInfer's ``[num_blocks, 2, page_size, ...]`` paged cache, and ``patch_flashinfer_metadata_builder`` exposes the dense paged metadata FlashInfer otherwise keeps only inside its planned wrappers. The calibration worker auto-selects the impl per layer (and reports the active impl class so the backend in use is verifiable); pass ``--attention_backend FLASHINFER`` to force it (FlashInfer needs a supported ``head_size``).
