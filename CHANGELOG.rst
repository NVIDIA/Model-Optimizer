...
- Add Minitron pruning support for Megatron-Core models with GatedDeltaNet (linear-attention) layers, such as Qwen3.5 (hybrid GatedDeltaNet + gated-attention) language models, including MoE variants, as well as Multi-Latent Attention (MLA) models such as DeepSeek. For GatedDeltaNet, gated-attention (``attention_output_gate``) and MLA layers, only ``hidden_size`` is pruned — their attention/linear-attention heads (and MLA latent ranks) are not pruned.
