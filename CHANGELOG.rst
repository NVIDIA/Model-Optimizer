...
- Add HuggingFace unified export of quantized Qwen3-VL and Qwen3.5-VL checkpoints (PTQ or QAD) via ``examples/megatron_bridge/export_quantized_megatron_to_hf.py``. Only the language model is quantized; the vision tower is copied from the source HuggingFace checkpoint. Qwen3.5-VL additionally covers GatedDeltaNet linear-attention layers and MoE shared experts, and requires ``--no_moe_grouped_gemm``. Gemma3-VL is still saved in Megatron checkpoint format only.
- Add ``--no_moe_grouped_gemm`` to ``examples/megatron_bridge/quantize.py``, ``distill.py`` and ``export_quantized_megatron_to_hf.py``, building MoE experts as ``SequentialMLP`` instead of the fused ``TEGroupedMLP``. The flag must match across all three, since the expert layout is baked into the Megatron checkpoint.
