...
- ``DistillationConfig.kd_loss_scale`` and ``DistillationConfig.skip_lm_loss`` (Megatron distillation plugin) are deprecated. ``kd_loss_scale`` is ignored with a ``FutureWarning``; ``skip_lm_loss`` passed without ``kd_loss_alpha`` is translated to an equivalent ``kd_loss_alpha`` (``True`` to ``1.0``) with a ``FutureWarning``, and is otherwise ignored in favor of ``kd_loss_alpha``. The ``--no_skip_lm_loss`` and ``--kd_loss_scale`` flags in ``examples/megatron_bridge/distill.py`` are likewise deprecated and ignored.
