...
- Fix ``fsdp2_aware_weight_update`` raising ``UnboundLocalError`` from its ``finally`` block when an exception (e.g. a CUDA OOM from the in-body ``unshard()``) fires before its bookkeeping locals are bound. The ``finally`` referenced ``fsdp_param_mapping`` / ``fsdp_param_group`` / ``root_module`` unconditionally, so the spurious ``UnboundLocalError`` replaced and masked the real error. These are now pre-bound and the writeback is guarded, so the original exception propagates unchanged. This makes export-time unshard failures on large (MoE) models debuggable.
