...
- Add per-element AutoQuantize selector scoring. Set ``auto_quantize.constraints.score_model: per_element`` in an AutoQuantize recipe to normalize selector coefficients by represented weight elements. The deprecated HF PTQ CLI flag ``--auto_quantize_score_model`` maps to the same recipe field.
