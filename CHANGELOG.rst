...
- Add **NVFP4 Conv3d weight export** to the unified Hugging Face diffusers export. Quantized ``Conv3d`` layers (e.g. the Wan 2.2 VAE ``WanCausalConv3d`` stack) are now serialized in the same logical flattened-K NVFP4 schema as Linear: each filter ``[O, C, kt, kh, kw]`` is flattened to ``[O, K_flat]`` in PyTorch-contiguous order, ``K_flat`` is padded to a multiple of the block size 16, and the result is stored as packed ``weight`` (``[O, K_pad / 2]`` uint8), per-block ``weight_scale`` (``[O, K_pad / 16]`` FP8 E4M3), and a scalar ``weight_scale_2``; ``input_scale`` is emitted when the activation amax is calibrated. Previously such modules fell through ``_process_quantized_modules`` unpacked. ``hide_quantizers_from_state_dict`` now strips quantizers from every module so no quantizer state (e.g. ``_amax``) is serialized. Scope is dynamic NVFP4 Conv3d; kernel-side layout preparation (scale swizzle, channel alignment) remains a downstream-runtime concern. See the Wan 2.2 VAE NVFP4 example in `examples/diffusers/README.md <https://github.com/NVIDIA/Model-Optimizer/tree/main/examples/diffusers>`__.
