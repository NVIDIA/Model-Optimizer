...
- Add ``SVDQuantConfig.alpha`` (diffusers example: ``--svdquant-alpha``): a fixed SmoothQuant-style migration strength for SVDQuant that skips the AWQ-Lite alpha search — one per-channel act-amax stats pass instead of AWQ-Lite's cache + candidate-search passes. ``alpha=1.0`` migrates activation outliers fully into the weights (which the SVD low-rank branch absorbs), following the SVDQuant paper. Default ``None`` keeps the AWQ-Lite search.
