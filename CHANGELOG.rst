...
- Add **LiLiCorr** speculative-decoding training (`arXiv:2608.20530 <https://arxiv.org/abs/2608.20530>`_): the parallel DFlash draft backbone plus a lightweight reranker over the candidate lattice it already produces, selected via ``dflash_architecture_config.projector_type=lilicorr``. The backbone is trained on per-position marginals, so its tokens are individually plausible yet jointly incoherent; instead of committing the per-slot argmax, the reranker keeps the top-``k`` candidates per slot, scores transitions between adjacent candidates as cosine similarities between an ``out`` and an ``in`` vector, and commits a path greedily. Trained jointly with the backbone on three weighted terms added to the block loss — a softmax over each slot's candidate scores that pushes the ground-truth candidate up and its competitors down, a hinge on the same scores, and a distractor penalty weighted by the target model's own logit gap. The weights are absolute, with no outer multiplier, so ``loss == origin_loss + lilicorr_loss`` holds exactly. Two published variants differ only in composition (``base``: ``dflash_lilicorr_w_ce=0.25``, ``w_margin=0``; ``margin``: ``0.125``/``0.125``; both ``w_pen=0.25``). Requires online training (the penalty reads the target's logits) and exports in the z-lab drafter format with ``architectures: ["LiLiCorrDraftModel"]``; recipe at ``modelopt_recipes/general/speculative_decoding/lilicorr.yaml``. Training and export only — the serving path is not in this repo.
- Add ``dflash_fp32_master_weights`` (default ``False``): keep the DFlash draft's parameters in fp32 while training under bf16 autocast, i.e. classic mixed precision with fp32 master weights. The parameter dtype decides AdamW's moment dtype, since they are allocated with ``zeros_like(p)``, and a bf16 second moment cannot decay at ``beta2=0.999`` — a step changes it by at most 0.1% while bf16's resolution there is ~0.4%, so it only grows and the effective step size only shrinks. Applies to every ``projector_type``; costs about 12 bytes per parameter instead of 6.
- **Activation checkpointing now reaches the DFlash draft.** ``DFlashModule`` declared no ``gradient_checkpointing`` attribute, so ``PreTrainedModel._set_gradient_checkpointing`` skipped it and ``training.gradient_checkpointing`` landed only on the frozen target — which stores no activations, and so reported the feature as enabled while saving nothing. The draft now honours the flag in its decoder-layer loop, defaulting to the non-reentrant implementation because the reentrant one can hang under ``ddp_find_unused_parameters=True``.
- Fix a **DDP hang** in DFlash training at scale. The draft's rotary ``inv_freq`` buffer was created on its first forward, and a rank whose batch has no valid anchor returns early without running the draft — leaving it one buffer short, so ``broadcast_buffers`` coalesced mismatched buffer lists across ranks and hung instead of raising. The buffer is now built during ``modify()``. Rank-count probabilistic, so single-node runs never saw it.
- Add optional **grouped sublayer convolutions for LiLiCorr**, reusing DFlash2's ``DFlashGroupedConv`` on the no-op sublayer seam ``DFlashDecoderLayer`` exposes. Enabled by putting ``conv_kernel_size`` and ``conv_group_size`` in ``dflash_architecture_config``; the two are optional but all-or-nothing, and ``conv_projection_init_std`` (default 0) makes the wrapper an exact identity at step 0 so a conv run begins as the plain reranker. Export emits both geometry keys, without which a serving loader defaults them to 0, builds no convolution modules and silently drops the conv tensors. Recipe at ``modelopt_recipes/general/speculative_decoding/lilicorr_conv.yaml``.
