...
- Add **N:M sparse softmax for diffusion models**: the ``triton_sparse_softmax`` method now reaches the diffusers and LTX-2 Triton attention backends (previously HF LLMs only), applying structured N:M sparsity (default 2:4) to attention scores inside the fused Triton kernel. The Wan 2.2 example (``examples/diffusers/sparsity/wan22_skip_softmax.py``) gains a ``--sparse-nm`` mode (with ``--sparsity-n`` / ``--sparsity-m``); cross-attention and the first/last ``--skip-first-last`` transformer blocks stay dense.
