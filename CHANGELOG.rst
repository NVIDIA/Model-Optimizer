...
- Add ``dflash_fp32_master_weights`` (default ``True``): keep the DFlash draft's parameters and Adam moments in fp32 while its matmuls run in the frozen base model's dtype (classic mixed precision). The draft supplies its own autocast, so the flag behaves the same under training, evaluation and generation. Yields 7–14% acceptance-length improvement over bf16-only training across all ``projector_type`` variants. The cost is memory, plus a doubled gradient all-reduce under DDP; set it to ``False`` to reduce both when training at the limit of a node's capacity.
