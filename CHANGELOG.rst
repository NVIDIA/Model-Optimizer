...
- Add **bypass distillation** as an optional stage in the Puzzletron heterogeneous pruning pipeline. When a ``bypass`` config section is present, pruned subblocks undergo knowledge-distillation fine-tuning between the prune and library-building stages; bypass-trained checkpoints take priority over pruned ones in the replacement library. The stage is skip-if-complete (checks manifest, status, config fingerprint, and realized symlink) so interrupted runs resume safely. The teacher checkpoint is auto-downloaded via ``huggingface_hub`` when the local path does not exist. A new ``target_num_kv_heads`` field is exposed as a MIP constraint to bound KV head count in the final architecture. See `examples/puzzletron/README.md <https://github.com/NVIDIA/Model-Optimizer/tree/main/examples/puzzletron>`_ for usage.
