...
- Fix quantizing a GPT-model ``output_layer`` (HuggingFace ``lm_head``) failing to checkpoint through ``examples/megatron_bridge``: megatron-core drops ``output_layer._extra_state``, where ModelOpt keeps quantizer state, so saving raised and loading silently restored the layer unquantized. The example scripts now retain that entry; the workaround disables itself once megatron-core keeps it.
