...
- Add ``dflash_fp32_master_weights`` (default ``True``): keep the DFlash draft's parameters and Adam moments in fp32 while matmuls run in bf16 (classic mixed precision). Yields 7–14% acceptance-length improvement over bf16-only training across all ``projector_type`` variants; set to ``False`` to reduce optimizer memory when training at the limit of a node's capacity.
