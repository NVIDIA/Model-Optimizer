...
- A recipe can now **delegate its whole body to another recipe** with a top-level ``$import``, overriding only ``metadata``. Every ``general/``, ``huggingface/`` and ``timm/`` recipe declares a ``modelopt-schema`` so it can be imported this way. ``modelopt_recipes/models/`` uses it to give every published checkpoint an entry at its own hub path: a thin alias when a portable recipe reproduces the release unchanged, a full body only when it deviates.
