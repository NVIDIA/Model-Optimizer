...
- Add group-boundary AutoQuantize scoring and make it the default for gradient scoring. Attention projections are scored at their parent attention output without grouping recipe decisions; fused, shared, and unfused MoE experts retain the established parent-MLP scoring boundary. Set ``score_boundary: local`` in an AutoQuantize recipe to restore leaf-output attention scoring. KL-divergence retains local scoring.
