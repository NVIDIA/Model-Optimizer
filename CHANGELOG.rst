...
- Resolve tied weights during HF checkpoint export by their declared **name** (from the model's ``_tied_weights_keys`` / ``tie_word_embeddings``) instead of by tensor ``data_ptr()``. Address identity misfires in several ways -- a freed address recycled by the allocator can falsely alias two unrelated weights, and the FSDP full-state-dict gather (and offload) materializes tied weights at distinct addresses so a genuine tie is missed and both copies are written. A single ``TiedGroupResolver`` (built once from the declarations) drives the input-amax sync and the authoritative dedup in ``postprocess_state_dict``, which drops a declared alias key whenever its canonical counterpart is present -- correct on 1 GPU and under multi-GPU FSDP alike (e.g. a 4-GPU MiniMax export). Both the per-module dense and fused-MoE ``data_ptr``-keyed dedup caches are removed: every tied weight is now packed independently and the duplicate is dropped by name. A ``(device, data_ptr, size)`` pass is retained only as a backstop for undeclared same-storage shares (which ``safetensors.save_file`` would otherwise refuse to serialize); it fires only for unquantized/unpacked shared weights, since quantized ties pack to distinct storage and are collapsed by name.
