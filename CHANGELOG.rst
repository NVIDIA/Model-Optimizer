...
- Add normalized and parent-boundary AutoQuant scoring. Set ``constraints["score_model"]="per_element"`` to normalize selector coefficients by represented weight elements, and use ``method="parent_recon"`` to score projection recipes by normalized reconstruction error at their parent attention or MLP output without grouping the attention recipe decisions. Shared-expert gate/up/down projections are grouped as one fused-MoE decision. The HF PTQ example exposes the scoring controls through ``--auto_quantize_score_model`` and ``--auto_quantize_score_boundary``.
