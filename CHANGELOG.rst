...
- ONNX quantization with explicit Autotune now retains the no-QDQ high-precision model when no tested INT8/FP8 placement meets the configured minimum TensorRT speedup (1.02x by default). The fallback is reported explicitly; quantization without Autotune is unchanged.
