...
- Add **experimental** ``/puzzletron`` Claude Code agent skill (``.agents/skills/puzzletron/``) with commands to run the MIP step or full pipeline, monitor progress, inspect teacher-vs-compressed accuracy, and add support for new models — including a step-by-step tutorial (``.agents/skills/puzzletron/adding_new_model_tutorial.md``). See `.agents/skills/puzzletron/README.md <https://github.com/NVIDIA/Model-Optimizer/tree/main/.agents/skills/puzzletron/README.md>`_.
