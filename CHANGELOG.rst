...
- Fix ``examples/hf_ptq`` PTQ and unified export from pack-quantized ``compressed-tensors`` checkpoints (including Kimi-K2 Thinking and Kimi-K2.6): checkpoint-dense Linear modules targeted by stale compression configs are now reconciled from the checkpoint tensor schema, packed expert weights are decompressed one layer at a time and included in weight calibration even when no tokens route to them, and export dedup uses the original packed tensor identity instead of allocator-recyclable transient buffers. This prevents group-size divisibility failures, missing expert weight ``amax`` values, and unrelated experts being falsely emitted as tied weights.
