...
For models too large for a single node, use `examples/llm_ptq/multinode_ptq.py` with FSDP2.

Multi-node SLURM template:

```bash
#!/bin/bash
#SBATCH --job-name=ptq-multinode
#SBATCH --account=<account>
#SBATCH --partition=<partition>
#SBATCH --nodes=<N>
#SBATCH --ntasks-per-node=1
...
#   SLURM_NODELIST, SLURM_NNODES, SLURM_PROCID, SLURM_LOCALID
...
srun \
    ...
    --no-container-mount-home \
    bash -c "
        pip install -e <modelopt_path>[hf] --quiet
        accelerate launch \
