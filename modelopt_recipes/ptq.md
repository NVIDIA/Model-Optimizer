...
- **`models/deepseek-ai/DeepSeek-V4-Pro-0813/ptq/nvfp4_experts_only`** mirrors
  ...
  `hf_ptq.py` flow.
- **`models/moonshotai/Kimi-K3/ptq/nvfp4_experts-kv_none_layerwise_export`** mirrors no
  ...
  NVFP4 W4A4 on the routed experts with the KV cache left unquantized, calibrated and
  **exported one decoder layer at a time**, so a run that outlasts its GPU session
  resumes without recalibrating or re-exporting finished layers. It scopes experts as `*.experts.*`
