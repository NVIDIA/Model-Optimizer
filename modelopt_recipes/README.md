...
`<checkpoint>/`) for a Hugging Face model or `timm/<architecture>/` for a timm model. If there's
no entry, fall back to `general/`. The presence of a model folder signals a recommended, tuned
recipe.
