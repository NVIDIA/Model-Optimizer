...
"""Unit tests for GPTQ implementation."""
...
@pytest.mark.parametrize(
    ...
    [
        (4, 32, torch.randn(32, 32).to("cuda"), True),  # random weight
        (
            4,
            16,
...
def test_gptq_updates(block_size, dim, model_weight, expect_weight_change):
    ...
    model.weight.data = model_weight.clone()
    input = torch.randn(2, 4, 32, dim).to("cuda")
    ...
    # Set name attribute (required for blockwise_weight_update)
