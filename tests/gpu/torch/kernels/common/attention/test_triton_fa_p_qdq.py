...
class TestSoftmaxQdqForward:
    ...
    @pytest.mark.parametrize(
        "qdq_kwargs",
        [{"p_qdq": "fp8"}, {"v_qdq": "nvfp4", "v_qdq_amax": 1.0}],
    )
    def test_skip_softmax_rejects_attention_quantization(self, qdq_kwargs):
        """The raw contiguous kernel rejects quantization with calibrated skip."""
        seq_len, num_heads, num_kv_heads, head_dim = 256, 4, 2, 64
