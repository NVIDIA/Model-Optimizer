...
# --- report-directory mode ------------------------------------------------------------
#
# $MODELOPT_REPORT_DIR writes the same information to plain files instead of talking to
# MLflow, for a caller (e.g. NeMo Evaluator Launcher) that already exports its own job as one
# MLflow run and would collide with a second, independent one ModelOpt opened for the same
# job. sys.modules["mlflow"] is set to None in these tests -- not mocked with FakeMlflow --
# so that any code path that still tried to `import mlflow` would fail loudly.


def test_report_dir_without_uri_uses_the_local_logger(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setitem(sys.modules, "mlflow", None)
    tracker = mlflow_utils.FakeQuantMlflowTracker(_worker(), QUANT_CONFIG)

    assert tracker.enabled
    assert isinstance(tracker._logger, mlflow_utils.LocalReportLogger)
    tracker.start()
    tracker.finish("FINISHED")

    assert yaml.safe_load((report_dir / "status.yaml").read_text())["status"] == "FINISHED"


def test_report_dir_takes_priority_over_a_tracking_uri(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setenv("MLFLOW_TRACKING_URI", URI)
    monkeypatch.setitem(sys.modules, "mlflow", None)
...
def test_report_dir_rank_nonzero_writes_nothing(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setitem(sys.modules, "mlflow", None)
    tracker = mlflow_utils.FakeQuantMlflowTracker(_worker(rank=3), QUANT_CONFIG)
...
def test_report_dir_start_writes_manifest_and_serving_params(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setitem(sys.modules, "mlflow", None)
    tracker = mlflow_utils.FakeQuantMlflowTracker(
    ...
    manifest = yaml.safe_load((report_dir / "manifest.yaml").read_text())
    assert manifest["params"]["quant_cfg"] == "NVFP4_DEFAULT_CFG"
    assert manifest["params"]["max_model_len"] == 4096
    assert manifest["tags"]["checkpoint_path"] == "/ckpts/Qwen3-0.6B"
    assert manifest["tags"]["tool"] == "vllm_serve_fakequant"


def test_report_dir_preset_quant_config_is_written(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setitem(sys.modules, "mlflow", None)
    tracker = mlflow_utils.FakeQuantMlflowTracker(_worker(), QUANT_CONFIG)
    ...
    written = yaml.safe_load((report_dir / "recipe/quant_cfg.yaml").read_text())
    assert written == {"quant_cfg": {"*weight_quantizer": {"num_bits": 4}}}


def test_report_dir_recipe_run_writes_the_resolved_recipe(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setitem(sys.modules, "mlflow", None)
    tracker = mlflow_utils.FakeQuantMlflowTracker(
        ...
        {
            **QUANT_CONFIG,
            "recipe_path": "general/ptq/nvfp4_default-kv_fp8_cast",
            "quant_cfg": None,
        },
    )

    tracker.start()
    tracker.log_quant_config(object())  # a recipe run must not also write quant_cfg.yaml
    tracker.finish("FINISHED")

    recipe = yaml.safe_load((report_dir / "recipe/resolved_recipe.yaml").read_text())
    assert recipe["metadata"]["recipe_type"] == "ptq"
    assert not (report_dir / "recipe/quant_cfg.yaml").exists()


def test_report_dir_quant_summary_is_written(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setitem(sys.modules, "mlflow", None)
    tracker = mlflow_utils.FakeQuantMlflowTracker(_worker(), QUANT_CONFIG)
    ...
    assert (
        report_dir / "summary/quant_summary.txt"
    ).read_text() == "2 TensorQuantizers found in model\n"


def test_report_dir_failed_step_updates_status_and_reraises(mlflow_utils, monkeypatch, tmp_path):
    report_dir = tmp_path / "modelopt"
    monkeypatch.setenv("MODELOPT_REPORT_DIR", str(report_dir))
    monkeypatch.setitem(sys.modules, "mlflow", None)
    tracker = mlflow_utils.FakeQuantMlflowTracker(_worker(), QUANT_CONFIG)
    ...
    assert yaml.safe_load((report_dir / "status.yaml").read_text())["status"] == "FAILED"


# --- LocalReportLogger, exercised directly ----------------------------------------------


def _status(report_dir):
    return yaml.safe_load((report_dir / "status.yaml").read_text())


def _manifest(report_dir):
    return yaml.safe_load((report_dir / "manifest.yaml").read_text())


def test_disabled_report_logger_writes_nothing(mlflow_utils, tmp_path):
    report_dir = tmp_path / "modelopt"
    logger = mlflow_utils.LocalReportLogger(report_dir, enabled=False)
...
def test_local_logger_start_writes_manifest_and_running_status(mlflow_utils, tmp_path):
    report_dir = tmp_path / "modelopt"
    logger = mlflow_utils.LocalReportLogger(report_dir)

    logger.start(params={"qformat": "nvfp4"}, tags={"tool": "vllm_serve_fakequant"})

    assert _status(report_dir) == {"status": "RUNNING"}
    manifest = _manifest(report_dir)
    assert manifest["params"] == {"qformat": "nvfp4"}
    assert manifest["tags"]["user"] == "tester"
    assert manifest["tags"]["tool"] == "vllm_serve_fakequant"


def test_local_logger_start_redacts_secret_named_params(mlflow_utils, tmp_path):
    report_dir = tmp_path / "modelopt"
    logger = mlflow_utils.LocalReportLogger(report_dir)

    logger.start(params={"api_key": "sekrit", "model": "x"})

    manifest = _manifest(report_dir)
    assert manifest["params"]["api_key"] == "***"
    assert manifest["params"]["model"] == "x"


def test_local_logger_start_writes_texts_as_individual_files(mlflow_utils, tmp_path):
    report_dir = tmp_path / "modelopt"
    logger = mlflow_utils.LocalReportLogger(report_dir)

    logger.start(texts={"recipe/resolved_recipe.yaml": "metadata:\n  recipe_type: ptq\n"})

    assert (report_dir / "recipe/resolved_recipe.yaml").read_text() == (
        "metadata:\n  recipe_type: ptq\n"
    )


def test_local_logger_log_text_writes_while_the_run_is_open(mlflow_utils, tmp_path):
    report_dir = tmp_path / "modelopt"
    logger = mlflow_utils.LocalReportLogger(report_dir)
    logger.start()
    ...
    assert (report_dir / "recipe/quant_cfg.yaml").read_text() == "quant_cfg: {}\n"


def test_local_logger_finish_updates_status_to_finished(mlflow_utils, tmp_path):
    report_dir = tmp_path / "modelopt"
    logger = mlflow_utils.LocalReportLogger(report_dir)
    logger.start()
    ...
    status = _status(report_dir)
    assert status["status"] == "FINISHED"
    assert "total_time_s" in status


def test_local_logger_only_files_this_run_produced_are_copied(mlflow_utils, tmp_path):
    """Mirrors MlflowRunLogger's freshness check: a rerun into a reused directory must not
    recopy a stale file left over from a previous, unrelated attempt."""
    report_dir = tmp_path / "modelopt"
    stale = tmp_path / ".quant_summary.txt"
    stale.write_text("from a previous run\n")
    logger = mlflow_utils.LocalReportLogger(report_dir)
    files = {"summary/quant_summary.txt": stale}

    logger.start(files=files)
    logger.finish("FAILED", files=files)

    assert not (report_dir / "summary/quant_summary.txt").exists()


def test_local_logger_report_dir_write_failure_is_fatal(mlflow_utils, tmp_path):
    """Unlike MlflowRunLogger, there is no server-reachability fallback here: this directory
    is the only copy, so a write failure must raise rather than warn."""
    blocked = tmp_path / "blocked"
    blocked.write_text("a file, not a directory")
    logger = mlflow_utils.LocalReportLogger(blocked / "modelopt")
