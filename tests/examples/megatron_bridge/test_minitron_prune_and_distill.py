...
def test_minitron_prune_and_distill(tmp_path: Path):
    n_gpus = torch.cuda.device_count()
    teacher_hf_path, teacher_model = create_tiny_qwen3_dir(
        tmp_path, with_tokenizer=True, return_model=True, num_hidden_layers=n_gpus
    )
    teacher_params = sum(p.numel() for p in teacher_model.parameters())

    # Prune teacher model with PP
    pruned_model_path = tmp_path / "pruned"
    prune_command_parts = extend_cmd_parts(
        ["torchrun", f"--nproc_per_node={n_gpus}", "prune_minitron.py"],
    ...
    # Distill pruned model with TP
    train_iters = 5
    distill_output_dir = tmp_path / "distill_output"
    distill_cmd_parts = extend_cmd_parts(
        ["torchrun", f"--nproc_per_node={n_gpus}", "distill.py", "--use_mock_data"],
        student_hf_path=pruned_model_path,
