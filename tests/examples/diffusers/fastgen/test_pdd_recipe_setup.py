...
def test_only_training_student_is_frozen_before_optimizer_collection(monkeypatch) -> None:
    class _Pipeline:
        @classmethod
        def from_pretrained(cls, *args, **kwargs):
            del cls, args, kwargs
            return SimpleNamespace(transformer=_PreparedStudent(out_features=32)), {}
    ...
    monkeypatch.setattr(
        ...
        lambda transformer: frozen.append(transformer) or (),
    )
    ...
    assert frozen == [student.transformer]
    assert teacher.transformer is not student.transformer
