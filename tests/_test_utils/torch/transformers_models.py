...
def get_tiny_deepseek_v3(**config_kwargs) -> PreTrainedModel:
    ...
    kwargs = {
        ...
        # parameter slot when ``config.topk_method == "noaux_tc"``; stamp it
        # explicitly so save → vLLM-load roundtrip doesn't ``KeyError`` on the
        # gate weight.
    ...
    # post-construction to survive ``save_pretrained``/``from_pretrained``.
