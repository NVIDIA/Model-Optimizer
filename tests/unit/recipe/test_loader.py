...
def test_load_recipe_mtp_boost_rejects_streaming(tmp_path):
    """Native-MTP boosting has no streaming feature source; reject it at load time."""
    ...
    with pytest.raises(Exception, match="does not support data.mode"):  # pydantic.ValidationError
        load_recipe(streaming)
