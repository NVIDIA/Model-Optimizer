...
import modelopt.torch.quantization._auto_quantize_shapley as _shapley
...
def test_coverage_inversion_recovers_forward_model():
    ...
    # corner exactly and preserve the allocation-relevant ratios between groups.
...
def test_reused_score_module_accumulates_every_invocation():
    """Calling one score module twice must attribute both calls."""
...
def test_scoring_setup_failure_restores_model(monkeypatch):
    """A partial scoring setup must not leave the model patched."""
    model = _OneLinear()
    model.fc.weight.requires_grad_(False)

    def fail_during_param_setup(_param):
        raise RuntimeError("injected scoring setup failure")

    monkeypatch.setattr(_shapley, "create_param_grad_clear_hook", fail_during_param_setup)

    with pytest.raises(RuntimeError, match="injected scoring setup failure"):
        _search(model)

    assert not model.fc.weight.requires_grad
    # Assert on the state the session actually mutates. Modules are converted before the
    # session patches them, so compare against each module's current class rather than a
    # pre-conversion snapshot; asserting absent attribute names would pass either way.
    for module in model.modules():
        assert getattr(module.forward, "__func__", None) is type(module).forward
