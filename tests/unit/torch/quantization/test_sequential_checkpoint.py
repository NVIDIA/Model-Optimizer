...
"""Unit tests for sequential calibration checkpoint save/resume."""
...
def test_full_run_creates_checkpoints(monkeypatch, tmp_path):
    """sequential_calibrate with checkpoint_dir creates correct layer dirs and manifest."""
    _register_test_discoverer(monkeypatch)
    model, forward_loop = _make_model_and_forward(n_layers=3)
    ckpt_dir = str(tmp_path / "ckpt")

    sequential_calibrate(model, forward_loop, _dummy_calib_func, checkpoint_dir=ckpt_dir)
...
def test_resume_matches_full_run(monkeypatch, tmp_path):
    ...
    sequential_calibrate(ref_model, forward_loop, _dummy_calib_func, checkpoint_dir=ckpt_dir)
    ...
    sequential_calibrate(resumed_model, forward_loop, _dummy_calib_func, checkpoint_dir=ckpt_dir)
...
def test_no_checkpoint_unchanged(monkeypatch):
    ...
    sequential_calibrate(model, forward_loop, _dummy_calib_func)
