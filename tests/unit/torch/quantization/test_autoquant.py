...
class _ParentBoundaryModel(torch.nn.Module):
    def __init__(self):
        super().__init__()
...
def test_auto_quantize_parent_reconstruction_score_is_normalized_mse():
    reference = torch.tensor([[1.0, 2.0]])
    quantized = torch.tensor([[2.0, 4.0]])

    score = quant_algorithms._get_parent_reconstruction_score(reference, quantized)

    expected = (quantized - reference).square().mean() / reference.square().mean()
    assert score.item() == pytest.approx(expected.item())
    with pytest.raises(ValueError, match="same shape"):
        quant_algorithms._get_parent_reconstruction_score(torch.ones(2), torch.ones(3))
    with pytest.raises(TypeError, match="expects a Tensor"):
        quant_algorithms._get_parent_reconstruction_score({"hidden": reference}, quantized)


def test_auto_quantize_parent_score_boundary_does_not_group_recipe_decisions():
    model = _ParentBoundaryModel()
    ...
    assert search_state["score_boundary"] == "parent"
    assert search_state["score_model"] == "per_element"
    assert search_state["method"] == "parent_recon"
...
@pytest.mark.parametrize("method", ["gradient", "parent_recon", "kl_div"])
...
def test_auto_quantize_checkpoint_rejects_score_boundary_mismatch(tmp_path):
    checkpoint_path = str(tmp_path / "autoquant_parent_boundary.pth")
    model = _ParentBoundaryModel()
    common = {
        "constraints": {"effective_bits": 8.0, "score_model": "per_element"},
        "quantization_formats": [
    ...
    mtq.auto_quantize(
        model,
        data_loader=[model.get_input()],
        score_boundary="parent",
        **common,
    )

    resumed_model = _ParentBoundaryModel()
