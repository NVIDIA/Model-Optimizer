...
class _TinyQwenTransformer(nn.Module):
    ...
    def __init__(self, *, packed_channels: int = 4, hidden_width: int = 5) -> None:
        ...
        self.backbone = nn.Linear(packed_channels, hidden_width)
        self.proj_out = nn.Linear(hidden_width, packed_channels)
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden = hidden + condition_value + 0.1 * timestep[:, None, None]
...
def _inputs(batch_size: int = 2):
    ...
    embeddings = torch.randn(batch_size, 3, 2)
    mask = torch.tensor([[1, 1, 0], [1, 0, 0]], dtype=torch.long)[:batch_size]
    negative_embeddings = torch.randn(batch_size, 3, 2)
...
def test_unfused_channel_major_output_maps_each_packed_head_in_order() -> None:
    ...
    with torch.no_grad():
        ...
        head_bias = torch.arange(16, dtype=torch.float32).reshape(4, 4) / 5
...
def test_fused_student_matches_explicit_packed_head_weighting() -> None:
    student = _TinyQwenTransformer()
    config = _config()
    projection = convert_qwen_image_to_pdd(student, config)
    ...
    all_heads = adapter.student_all_heads(student, state, time, condition=condition)
    student.calls.clear()
    ...
    expected = torch.einsum("n,bnchw->bchw", coefficients, all_heads[:, 1:4])
...
def test_teacher_cfg_and_packed_token_norm_rescale_match_direct_reference() -> None:
    ...
    conditional = teacher.calls[0]["output"].float()
    unconditional = teacher.calls[1]["output"].float()
...
def test_raw_head_reference_uses_independent_linear_outputs() -> None:
    ...
    state, time, condition, _ = _inputs(batch_size=1)
    embeddings, mask = condition
    packed = pack_latents(state)
    hidden = torch.tanh(student.backbone(packed))
    hidden = hidden + embeddings.mean(dim=(1, 2), keepdim=True)
    hidden = hidden + 0.01 * mask.sum(dim=1, keepdim=True).unsqueeze(-1)
    hidden = hidden + 0.1 * time[:, None, None]
