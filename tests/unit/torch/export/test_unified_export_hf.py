...
def test_postprocess_keeps_untied_projection_of_partially_tied_container():
    """When only ``gate_up_proj`` of a fused-experts container is tied (``down_proj``
    independent), the container is not fully tied, so its per-expert keys are not swept and
    the independent ``down_proj`` survives (conservative: keep both)."""
    ...
    assert set(out) == set(sd)  # partially-tied container: nothing swept, both sides kept
