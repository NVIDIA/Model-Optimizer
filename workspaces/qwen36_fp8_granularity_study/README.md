...
within-scope rankings, tokenwise metrics, equal-document bootstrap intervals, quantizer-MSE family
and highest-layer summaries, logical FP8 weight cost, and phase wall times.

The evaluation is a deterministic 32-document slice from the same staged CNN/DailyMail training
split used for calibration. It is a controlled numerical screen, not a claim of broad task or
corpus generalization. The W8A8 candidates are format-policy bundles: temporal scale policy is
different between per-tensor and dynamic block formats, and MXFP8 changes both block size and
scale encoding. The W8A16 controls remove activation quantization but do not eliminate the MXFP8
block-size/scale-encoding confound.
