...
def _render_experiment_v2(
    ...
    if config.mip_configured:
        ...
        if budget == "smoke":
            for run in _mapping(rendered["mip"].get("runs")).values():
                solver = _mapping(run.get("solver"))
                solver["num_solutions"] = min(int(solver.get("num_solutions", 2) or 2), 2)
                run["solver"] = solver
        ...
        if budget == "smoke":
            for flow in rendered["post_mip"]["flows"].values():
                for node in _mapping(flow.get("nodes")).values():
                    ...
                    if node_type == "downstream_evaluation" and "profile" not in node_config:
                        node_config["limit"] = min(int(node_config.get("limit", 8) or 8), 8)
                    elif node_type == "evaluation":
                        node_config["eval_samples"] = min(
                            int(node_config.get("eval_samples", 2) or 2), 2
                        )
                        node_config["block_size"] = min(
                            int(node_config.get("block_size", 512) or 512), 512
                        )
                    elif node_type == "aiperf":
                        node_config["input_tokens"] = min(
                            int(node_config.get("input_tokens", 100) or 100), 100
                        )
                        node_config["output_tokens"] = min(
                            int(node_config.get("output_tokens", 80) or 80), 80
                        )
                        node_config["request_count"] = min(
                            int(node_config.get("request_count", 1) or 1), 1
                        )
                        extra_inputs = _mapping(node_config.get("extra_inputs"))
                        if extra_inputs:
                            extra_inputs["min_tokens"] = min(
                                int(extra_inputs.get("min_tokens", 80) or 80), 80
                            )
                            node_config["extra_inputs"] = extra_inputs
                    elif node_type == "global_kd":
                        node_config["max_steps"] = min(int(node_config.get("max_steps", 2) or 2), 2)
                        node_config["global_batch_size"] = 1
                        node_config["checkpoint_every_steps"] = 2
                    elif node_type == "filter":
                        node["top_k"] = min(int(node.get("top_k", 1) or 1), 1)
                    if node_config:
...
def _bundle_readme(
    ...
    lines = [
        "# Puzzletron campaign",
        "",
        "Generated by `puzzletron.py setup`. The wizard did not launch any jobs.",
        (
            "Run setup and orchestrator commands from the Python 3.10+ controller venv "
            f"described in `{Path(repository) / 'examples/puzzletron/docs/environment_setup.md'}`."
        ),
    ]
    ...
    if acquisition:
        ...
        lines.extend(
            (
                ...
                "",
                (
                    "This is a worker command. Run it inside the worker venv or container "
                    "configured in `smoke/runner.yaml` and `production/runner.yaml`, not in "
                    "the lightweight controller venv."
                ),
    ...
    if acquisition.get("adapter") == "nemotron_vlm_v2":
        evaluation_guide = Path(repository) / (
            "examples/puzzletron/docs/vlm_checkpoint_evaluation.md"
        )
        lines.extend(
            (
                "",
                f"For VLM evaluator caches, follow `{evaluation_guide}`.",
            )
        )
