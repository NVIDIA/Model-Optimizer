...
Run `nel --version`. The maintained task contracts are tested with
`nemo-evaluator-launcher==0.2.6`; use an isolated controller environment with
that version when the launcher is missing or older. If the user has an existing
config, skip to Step 8 (optionally review for `???` and quantization flags first).

`examples/llm_eval/task_contracts.yaml` is the single executable source for the
maintained task definitions. Materialize them with
`examples/llm_eval/nel_config.py`; recipe files contain operational and
score-extraction notes, not a second copy of task YAML.
...
3. Run `examples/llm_eval/nel_config.py --base-config <base>` once, adding one
   `--task <task>` option for every selected task. Apply the operational notes
   from the matching recipes; do not hand-author another task contract.
...
2. For any task with a `recipes/tasks/` reference, read its operational notes,
   then select the task through `examples/llm_eval/nel_config.py`. Task names,
   containers, prompts, splits, and repeat counts come from
