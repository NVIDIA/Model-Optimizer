...
Classes:
--------
- InsertionPoint: Abstract base class for pattern-relative insertion points
...
from modelopt.onnx.op_types import (
    get_autotuner_quantizable_operations,
    get_autotuner_skip_ops,
    get_bool_operations,
)
...
class InsertionPoint(ABC):
    ...
    Defines the common interface for all insertion point types that specify
    where to insert Q/DQ pairs within region patterns. Concrete implementations
    include:
    - NodeInputInsertionPoint: Insertion at node inputs
    - ChildRegionInputInsertionPoint: Insertion at child region input boundaries
    - RegionOutputInsertionPoint: Insertion at region/node outputs

    All insertion points support:
    - Serialization to/from dictionaries
    - Resolution to actual tensor names for specific region instances
    - Collection of valid insertion points from regions
    """
    ...
    def collect_from_region(region: "Region", graph: gs.Graph) -> list["InsertionPoint"]:
        ...
        Returns:
            List of insertion point objects representing valid insertion locations
        """
        ...
    ...
    for node in nodes:
        for input_idx, inp in enumerate(node.inputs):
            if hasattr(inp, "name") and inp.name == tensor_name:
                ...
                if node.op in get_bool_operations():
                    return True
...
def has_quantizable_operations(region: "Region", graph: gs.Graph) -> bool:
    ...
    if region.get_type() == RegionType.LEAF:
        ...
        return bool(region_ops.intersection(get_autotuner_quantizable_operations()))
    return True
...
def resolve_region_io_insertion_points(
    ...
    **Use Cases:**
    ...
    if tensor_name in tensor_users_map:
        node_indices.update(tensor_users_map[tensor_name])
