...
def distillation_overfit_stage(config: dict[str, Any], manifest: StageManifest):
    ...
    for solution in solutions:
        ...
        kd = {
            ...
            "force_hf": False,
            "domain": "llm",
            "tp": int(stage_cfg.get("tp", 2)),
            "cp": int(stage_cfg.get("cp", 2)),
            "pp": int(stage_cfg.get("pp", 2)),
            "dp": int(stage_cfg.get("dp", 2)),
            "ep": int(stage_cfg.get("ep", 1)),
            "sequence_parallel": bool(stage_cfg.get("sequence_parallel", True)),
