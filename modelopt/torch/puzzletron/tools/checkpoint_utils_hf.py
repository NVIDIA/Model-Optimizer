...
def _save_checkpoint_from_distributed_shards(
    ...
    for relative_filename in sorted(set(weight_map.values())):
        local_file_tensors = {
            ...
            if owner_by_key.get(key) == dist_utils.rank() and weight_map.get(key) == relative_filename
        }
...
def _copy_auto_map_code_files(model_config: PretrainedConfig, checkpoint_dir: Path) -> None:
    ...
    module_names = {
        class_ref.split(".")[0] for class_ref in _iter_auto_map_class_refs(model_config.auto_map)
    }
