...
def load_block_configs(
    ...
    logger.info("Generated %d block_configs via converter '%s'", len(configs), converter_name)
    return configs
...
def _normalize_block_configs(raw) -> list[dict]:
    ...
    for bc in raw:
        ...
        elif dataclasses.is_dataclass(bc) and not isinstance(bc, type):
