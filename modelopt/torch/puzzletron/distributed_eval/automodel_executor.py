...
class AutoModelReplaceBlockExecutor:
    ...
    def evaluate(self, request: EvaluationRequest) -> EvaluationResult | None:
        ...
        counts: dict[str, int | float] = {
            str(name): len(value.get("per_sample", []))
    ...
    def _score(self, prune_target: dict | list[dict] | None) -> dict[str, Any] | None:
        ...
        with ExitStack() as stack:
            ...
            for batch_idx, (hidden, targets) in enumerate(recipe.iterate_captures()):
                if hidden is None:
                    continue
                if candidate_lm_head is None:
                    raise RuntimeError("AutoModel scoring requires a candidate LM-head weight")
