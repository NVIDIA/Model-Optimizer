...
"""Custom mapping from Megatron Core models to GLM-5.3-Flash (``glm5_next``) Hugging Face models.

Megatron-Bridge builds each HF decoder layer as two physical layers wrapped in mHC hyper-connections:
attention (KDA or NoPE-MLA DSA) at ``2N`` and the dense MLP / MoE at ``2N+1``. The MTP layer sits at
HF index ``num_hidden_layers`` under the same names. The vision tower is copied from HF.
"""
