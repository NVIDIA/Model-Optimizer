...
def _tied_weight_modules(model: nn.Module) -> list[str]:
    """Modules sharing a weight with another.

    Grouped by name, which survives offload: a ``data_ptr`` grouping sees nothing when the
    weights are on meta and would pass vacuously. Any parameter the tie map does not name
    falls back to ``data_ptr``.
    """
    tied_map = TiedWeightMap(model)
    groups: dict[str, list[str]] = {}
    by_ptr: dict[int, list[str]] = {}
    for name, module in model.named_modules():
        # _parameters, not getattr: a wrapper forwarding .weight to a child (PEFT
        # base_layer) would otherwise group with the child it borrows from.
