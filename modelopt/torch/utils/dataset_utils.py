...
__all__ = [
    ...
    "get_dataset_samples",
    "get_dataset_samples_with_holdout",
...
def _load_streaming_dataset(
    ...
    *,
    ...
    This is a shared helper for :func:`get_dataset_samples` and
    :func:`get_dataset_samples_with_holdout`.
...
def get_dataset_samples_with_holdout(
    dataset_name: str,
    num_samples: int,
    num_holdout: int,
    *,
    ...
    """Load calibration and holdout samples from a dataset in a single streaming pass.

    Samples are loaded sequentially: the first ``num_samples`` non-empty samples go to the
    calibration set, and the next ``num_holdout`` non-empty samples go to the holdout set.
    Empty samples are skipped without counting toward either quota.  This guarantees zero
    overlap between the two sets by construction.

    Args:
        dataset_name: Name or HuggingFace path of the dataset to load, a local directory path,
            or a path to a ``.jsonl`` file.
        num_samples: Number of non-empty samples for the calibration set.
        num_holdout: Number of non-empty samples for the holdout set.
        apply_chat_template: Whether to apply the chat template to the samples.
        tokenizer: Tokenizer to use for applying the chat template.
        split: Override the split(s) to load.

    Returns:
        A tuple ``(calib_samples, holdout_samples)``.
    """
    if dataset_name.endswith(".jsonl"):
        all_samples = get_jsonl_text_samples(dataset_name, num_samples + num_holdout, key="text")
        calib = all_samples[:num_samples]
        holdout = all_samples[num_samples:]
        if len(calib) < num_samples:
            warn(
                f"JSONL file only had {len(calib)} samples, "
                f"fewer than the requested {num_samples} calibration samples."
            )
        if len(holdout) < num_holdout:
            warn(
                f"JSONL file only had {len(holdout)} holdout samples after calibration, "
                f"fewer than the requested {num_holdout}."
            )
    ...
    calib_per_split = [num_samples // len(dataset_splits)] * len(dataset_splits)
    calib_per_split[-1] += num_samples - sum(calib_per_split)

    holdout_per_split = [num_holdout // len(dataset_splits)] * len(dataset_splits)
    holdout_per_split[-1] += num_holdout - sum(holdout_per_split)

    calib_samples: list[str] = []
    holdout_samples: list[str] = []
    for dataset, calib_n, holdout_n in zip(dataset_splits, calib_per_split, holdout_per_split):
        split_calib: list[str] = []
        split_holdout: list[str] = []
        for sample in dataset:
            text = _preprocess(sample)
            if not text:
                continue
            if len(split_calib) < calib_n:
                split_calib.append(text)
            elif len(split_holdout) < holdout_n:
                split_holdout.append(text)
            else:
                break
        if len(split_calib) < calib_n:
            warn(
                f"Dataset split exhausted: only {len(split_calib)} calibration samples "
                f"collected, fewer than the requested {calib_n}."
            )
        if len(split_holdout) < holdout_n:
            warn(
                f"Dataset split exhausted: only {len(split_holdout)} holdout samples "
                f"collected, fewer than the requested {holdout_n}."
            )
        calib_samples.extend(split_calib)
        holdout_samples.extend(split_holdout)

    return calib_samples, holdout_samples
...
def _validate_no_overlap(calib_ids: torch.Tensor, holdout_ids: torch.Tensor) -> None:
    """Assert that calibration and holdout ``input_ids`` have no shared rows."""
    calib_set = {tuple(row.tolist()) for row in calib_ids}
    holdout_set = {tuple(row.tolist()) for row in holdout_ids}
    overlap = calib_set & holdout_set
    assert not overlap, f"Calibration and holdout data overlap: {len(overlap)} shared sequences."
    print(f"Validated no overlap: {len(calib_set)} calib, {len(holdout_set)} holdout, 0 shared.")


def _dedup_tokenized(
    calib_encoded: dict[str, torch.Tensor],
    holdout_encoded: dict[str, torch.Tensor],
) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor]]:
    """Remove duplicate tokenized sequences within calib and between calib/holdout.

    After truncation, different texts can produce identical ``input_ids`` (e.g.
    same problem statement with different solutions).  Feeding duplicates to
    calibration wastes samples and skews statistics; keeping them in holdout
    contaminates evaluation.

    Dedup order:
        1. Remove duplicates within calibration (keep first occurrence).
        2. Remove holdout rows that match any remaining calibration row.

    Returns filtered copies of both encoded dicts.
    """
    # Dedup within calib.
    seen: set[tuple] = set()
    calib_keep: list[int] = []
    for i, row in enumerate(calib_encoded["input_ids"]):
        key = tuple(row.tolist())
        if key not in seen:
            seen.add(key)
            calib_keep.append(i)
    n_calib_dupes = len(calib_encoded["input_ids"]) - len(calib_keep)

    # Remove holdout rows that collide with calib.
    holdout_keep: list[int] = []
    for i, row in enumerate(holdout_encoded["input_ids"]):
        if tuple(row.tolist()) not in seen:
            holdout_keep.append(i)
    n_holdout_overlap = len(holdout_encoded["input_ids"]) - len(holdout_keep)

    if n_calib_dupes or n_holdout_overlap:
        print(
            f"Dedup: removed {n_calib_dupes} duplicate calib and "
            f"{n_holdout_overlap} overlapping holdout sequences "
            f"({len(calib_keep)} calib, {len(holdout_keep)} holdout remaining)."
        )
    else:
        print(f"No duplicates: {len(calib_keep)} calib, {len(holdout_keep)} holdout, 0 shared.")

    def _index(encoded: dict[str, torch.Tensor], indices: list[int]) -> dict[str, torch.Tensor]:
        idx = torch.tensor(indices, dtype=torch.long)
        return {k: v[idx] for k, v in encoded.items()}

    return _index(calib_encoded, calib_keep), _index(holdout_encoded, holdout_keep)


def get_calib_and_holdout_dataloaders(
...
) -> tuple[DataLoader, Path | None]:
    """Create a calibration DataLoader and save holdout data to disk.

    On the first call the function streams ``calib_size + holdout_size`` samples
    from ``dataset_name``, tokenizes both splits, saves them as ``.pt`` files
    under ``save_dir``, and returns the calibration DataLoader.

    On subsequent calls, if both ``calib.pt`` and ``holdout.pt`` already exist
    under ``save_dir``, the function reloads directly from disk (skipping the
    dataset download) and validates that calibration and holdout have zero
    overlapping ``input_ids`` rows.

    Args:
        dataset_name: Name(s) of the dataset(s) to load.
        tokenizer: HuggingFace tokenizer instance.
        batch_size: Batch size for the calibration DataLoader.
        calib_size: Number of calibration samples (per dataset if list).
        holdout_size: Total number of holdout samples.
        max_sample_length: Maximum token length per sample.
        device: Target device for the calibration DataLoader tensors.
        include_labels: Whether to include labels in the calibration DataLoader.
        apply_chat_template: Whether to apply the chat template.
        save_dir: Directory to save/load ``calib.pt`` and ``holdout.pt``.

    Returns:
        A ``(calib_dataloader, holdout_path)`` tuple.
    """
    ...
    calib_pt = save_path / "calib.pt" if save_path else None
    holdout_pt = save_path / "holdout.pt" if save_path else None

    if calib_pt and holdout_pt and calib_pt.is_file() and holdout_pt.is_file():
        # ---- Reload from previously saved .pt files ----
        print(f"Loading calibration data from {calib_pt}")
        calib_data = torch.load(calib_pt, map_location="cpu", weights_only=True)
        print(f"Loading holdout data from {holdout_pt}")
        holdout_data = torch.load(holdout_pt, map_location="cpu", weights_only=True)

        _validate_no_overlap(calib_data["input_ids"], holdout_data["input_ids"])

        if device:
            calib_data = {k: v.to(device) for k, v in calib_data.items()}
        if include_labels:
            calib_data["labels"] = torch.where(
                calib_data["attention_mask"] > 0.5,
                calib_data["input_ids"],
                -100,
            )
        else:
            calib_data = {"input_ids": calib_data["input_ids"]}

        calib_dl = DataLoader(_CustomDataset(calib_data), batch_size=batch_size, shuffle=False)
        return calib_dl, holdout_pt

    # ---- Fresh load: stream calib + holdout, tokenize, save ----
    all_calib_text: list[str] = []
    all_holdout_text: list[str] = []
    holdout_per_ds = [holdout_size // len(dataset_name)] * len(dataset_name)
    holdout_per_ds[-1] += holdout_size - sum(holdout_per_ds)

    for ds_name, n_calib, n_holdout in zip(dataset_name, calib_size, holdout_per_ds):
        calib_text, holdout_text = get_dataset_samples_with_holdout(
            ds_name,
            num_samples=n_calib,
        ...
        all_calib_text.extend(calib_text)
        all_holdout_text.extend(holdout_text)

    calib_encoded = tokenizer(
        all_calib_text,
        return_tensors="pt",
        padding=True,
        truncation=True,
    ...
    calib_encoded, holdout_encoded = _dedup_tokenized(calib_encoded, holdout_encoded)
    _validate_no_overlap(calib_encoded["input_ids"], holdout_encoded["input_ids"])
    ...
    if save_path:
        ...
        torch.save(
            {
                "input_ids": calib_encoded["input_ids"],
                "attention_mask": calib_encoded["attention_mask"],
            },
            calib_pt,
        )
        print(f"Saved {len(calib_encoded['input_ids'])} calib samples to {calib_pt}")
        torch.save(
            {
                "input_ids": holdout_encoded["input_ids"],
                "attention_mask": holdout_encoded["attention_mask"],
            },
            holdout_pt,
        )
        print(f"Saved {len(holdout_encoded['input_ids'])} holdout samples to {holdout_pt}")

    # Build calib dataloader.
    if device:
        calib_encoded = {k: v.to(device) for k, v in calib_encoded.items()}
    if include_labels:
        calib_encoded["labels"] = torch.where(
            calib_encoded["attention_mask"] > 0.5,
            calib_encoded["input_ids"],
            -100,
        )
    else:
        calib_encoded = {"input_ids": calib_encoded["input_ids"]}

    calib_dl = DataLoader(_CustomDataset(calib_encoded), batch_size=batch_size, shuffle=False)
    return calib_dl, holdout_pt
