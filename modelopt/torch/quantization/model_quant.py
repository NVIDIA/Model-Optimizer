...
from .algorithms import (
    ...
    AutoQuantizeParentReconSearcher,
    QuantRecipe,
)
...
def auto_quantize(
    ...
    Args:
        ...
        method: Method to use for estimating sensitivity loss. Higher loss indicates greater sensitivity
            ...
            ``"parent_recon"`` (uses normalized parent-output reconstruction and requires only
            ``forward_step``), and
        ...
        score_boundary: Boundary used to measure perturbations. ``"local"``
            preserves the existing per-module behavior. ``"parent"`` scores attention
            projections at their parent self-attention or linear-attention output and scores
            fused/shared MoE projections at the parent MLP output. This does not group modules
            or force them to use the same quantization format. Defaults to ``"parent"`` for
            ``method="parent_recon"`` and ``"local"`` otherwise.
    ...
    if method == "gradient":
        ...
        raise ValueError(
            f"Invalid method: {method}. Valid options are 'gradient', 'parent_recon', or 'kl_div'."
        )

    score_boundary = score_boundary or ("parent" if method == "parent_recon" else "local")
