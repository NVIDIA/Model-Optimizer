...
def auto_quantize(
    ...
    methods (default), group-output reconstruction, or KL divergence loss, controlled by the
    ``method`` parameter.
    ...
    Args:
        ...
        score_boundary: Boundary used to measure perturbations. ``"local"``
            ...
            decisions or force modules to use the same quantization format. Defaults to
            ``"group"`` for ``method="group_recon"`` and ``"local"`` otherwise.
