...
* **Checkpoint saving** — :func:`save_sequential_checkpoint` attaches progress
  to the model and delegates to the registered saver.
"""
...
def detect_sequential_resume_layer(model: nn.Module, num_layers: int) -> int:
    """Read checkpoint progress from the model and return the layer index to resume from.

    Returns ``0`` for a fresh run with no checkpoint present.
