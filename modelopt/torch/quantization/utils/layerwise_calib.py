...
def _save_layer(
    ...
    if next_inputs is not None:
        final = os.path.join(checkpoint_dir, "next_inputs.pt")
        tmp = final + ".tmp"
...
class _CheckpointState:
    ...
    def from_folder(cls, checkpoint_dir: str | None, num_layers: int) -> _CheckpointState | None:
        ...
        # Drop any orphan .tmp from a crashed mid-save; the committed file is authoritative.
        with contextlib.suppress(FileNotFoundError):
            os.remove(os.path.join(checkpoint_dir, "next_inputs.pt.tmp"))
    ...
    def setup_resume(self, layers: nn.ModuleList) -> list | None:
        ...
        next_inputs_path = os.path.join(self.checkpoint_dir, "next_inputs.pt")
