...
def _next_inputs_path(checkpoint_dir: str) -> str:
    return os.path.join(checkpoint_dir, "next_inputs.pt")


def _save_layer_artifacts(
    checkpoint_dir: str,
    idx: int,
    weights: dict,
    qstate: dict,
    ...
    """Write the per-layer durable artifacts (weights, qstate, output_meta)."""
...
def _save_next_inputs(checkpoint_dir: str, next_inputs: list) -> None:
    """Atomically replace top-level ``next_inputs.pt`` with *next_inputs*.

    Writes ``next_inputs.pt.tmp`` then ``os.replace``s it into the final name,
    so the previous file is only superseded once the new one is fully written.
    """
    final = _next_inputs_path(checkpoint_dir)
    tmp = final + ".tmp"
    torch.save(next_inputs, tmp)
...
class _CheckpointState:
    ...
    def from_folder(cls, checkpoint_dir: str | None, num_layers: int) -> _CheckpointState | None:
        ...
        # Discard any orphan ``next_inputs.pt.tmp`` from a previous crash mid-save.
        # The committed ``next_inputs.pt`` (if any) is the one the manifest refers to;
        # we cannot tell from the bare ``.tmp`` whether it was pre- or post-commit, so
        # the safe move is to drop it and trust the on-disk manifest + final file.
        with contextlib.suppress(FileNotFoundError):
            os.remove(_next_inputs_path(checkpoint_dir) + ".tmp")
    ...
    def setup_resume(self, layers: nn.ModuleList) -> list | None:
        ...
        next_inputs_path = _next_inputs_path(self.checkpoint_dir)
        if not os.path.isfile(next_inputs_path):
            raise FileNotFoundError(
                f"Cannot resume: {next_inputs_path} missing "
                f"(needed for layer {self.start_layer} after completed layer {last_ckpt})"
            )
        # weights_only=False is safe: file is internally generated by _save_next_inputs
    ...
    def save(
        ...
        Only a single top-level ``next_inputs.pt`` is kept on disk at any time;
        it is atomically replaced via ``os.replace`` so the previous file is
        only superseded once the new one is fully written. The manifest is
        committed last and remains the authoritative resume signal.
        ...
        if next_layer_inputs is not None:
            _save_next_inputs(self.checkpoint_dir, _move_to_device(next_layer_inputs, _cpu))

        _write_manifest(self.checkpoint_dir, layer_idx, self.num_layers)
