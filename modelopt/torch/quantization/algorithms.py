...
_SELF_ATTN_PARENT_RE = re.compile(r"^((?:.*\.)?self_attn)\.(?:q_proj|k_proj|v_proj|o_proj)$")
_LINEAR_ATTN_PARENT_RE = re.compile(
    r"^((?:.*\.)?linear_attn)\.(?:in_proj_qkv|in_proj_z|in_proj_a|in_proj_b|out_proj)$"
)
_FUSED_EXPERTS_PARENT_RE = re.compile(r"^((?:.*\.)?mlp)\.experts$")

AUTO_QUANTIZE_SCORE_BOUNDARY_LOCAL = "local"
AUTO_QUANTIZE_SCORE_BOUNDARY_PARENT = "parent"
AUTO_QUANTIZE_SCORE_BOUNDARIES = frozenset(
    {AUTO_QUANTIZE_SCORE_BOUNDARY_LOCAL, AUTO_QUANTIZE_SCORE_BOUNDARY_PARENT}
)
...
def _self_attn_parent_score_module(_model, name: str) -> str | None:
    match = _SELF_ATTN_PARENT_RE.match(name)
    return match.group(1) if match else None


def _linear_attn_parent_score_module(_model, name: str) -> str | None:
    match = _LINEAR_ATTN_PARENT_RE.match(name)
    return match.group(1) if match else None
...
class _AutoQuantizeBaseSearcher(BaseSearcher, ABC):
    ...
    def _get_score_module_rules(self):
        rules = []
        if self.config["score_boundary"] == AUTO_QUANTIZE_SCORE_BOUNDARY_PARENT:
            rules.extend(
                [
                    _self_attn_parent_score_module,
                    _linear_attn_parent_score_module,
    ...
    def before_search(self):
        ...
        if self.method_name not in {"gradient", "parent_recon"} and (
            ...
            raise ValueError(
                "score_model='per_element' and score_boundary='parent' are supported only "
                "with method='gradient' or method='parent_recon'."
            )
    ...
    def run_search(self):
        ...
        self.best["constraints"] = {
            ...
            "score_model": self.constraints["score_model"],
        }
    ...
    def run_search_with_stats(self, max_weight_size, verbose=False):
        ...
        for lower_bound in self._get_search_lower_bounds():
            ...
            requires_objective_rescaling = (
                ...
                == AUTO_QUANTIZE_SCORE_MODEL_PER_ELEMENT
                or self.method_name == "parent_recon"
            )
            max_abs_score = (
                max(
            ...
            if max_abs_score > 0.0:
                ...
                # tolerance. Parent reconstruction scores are normalized and
...
def _get_primary_output_tensor(output: Any) -> torch.Tensor:
    ...
    if isinstance(output, tuple | list) and output and isinstance(output[0], torch.Tensor):
        return output[0]
    raise TypeError(
        "Parent reconstruction scoring expects a Tensor or a tuple/list whose first item "
        f"is a Tensor, got {type(output)!r}."
    )


def _get_parent_reconstruction_score(reference_output: Any, quantized_output: Any) -> torch.Tensor:
    """Return normalized MSE between reference and quantized parent outputs."""
    ...
    if reference.shape != quantized.shape:
        raise ValueError(
            "Reference and quantized parent outputs must have the same shape, got "
...
class AutoQuantizeParentReconSearcher(AutoQuantizeGradientSearcher):
    """AutoQuantize searcher using normalized parent-output reconstruction error."""

    method_name = "parent_recon"

    def sanitize_search_config(self, config: SearchConfig | None) -> SearchConfig:
        """Ignore backward-only inputs and require the parent score boundary."""
        config = dict(config or {})
        for ignored_key in ("score_func", "loss_func", "forward_backward_step"):
            if config.get(ignored_key) is not None:
                warnings.warn(f"`{ignored_key}` is ignored for parent_recon auto_quantize.")
            config.pop(ignored_key, None)
        config = _AutoQuantizeBaseSearcher.sanitize_search_config(self, config)
        if config["score_boundary"] != AUTO_QUANTIZE_SCORE_BOUNDARY_PARENT:
            raise ValueError("method='parent_recon' requires score_boundary='parent'.")
        return config
    ...
    def estimate_sensitivity_scores(self) -> None:
            ...
            for hparam in hparams:
                try:
                    for recipe in hparam.choices:
                        ...
                        score = _get_parent_reconstruction_score(reference_output, quantized_output)
        ...
        try:
            self._run_func(
                self.config["forward_step"],
                num_iters=self.config["num_score_steps"],
                desc="Estimating parent reconstruction scores",
            )
        finally:
...
def _resolve_best_recipe(search_state, constraints, verbose=False):
    ...
    elif method == "parent_recon":
        searcher = AutoQuantizeParentReconSearcher()
    elif method == "kl_div":
        searcher = AutoQuantizeKLDivSearcher()
    else:
        raise ValueError(
            f"Unknown autoquant search method: {method!r}. "
            "Expected 'gradient', 'parent_recon', or 'kl_div'."
        )
