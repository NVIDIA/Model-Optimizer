...
def _fwd_kernel(
    ...
    for start_n in range(start_n_limit, end_n_limit, BLOCK_N):
        ...
        do_update = True
        if SKIP_SOFTMAX:
            qk_row_max = tl.max(qk, 1)
            max_diff = tl.max(qk_row_max - m_i, 0)
            do_update = max_diff >= log2_threshold
...
def context_attention_fwd(
    ...
    Args:
        ...
        skip_threshold: Threshold in (0, 1) for skip-softmax sparsity; None disables.
    """
    ...
    skip_softmax = skip_threshold is not None
    log2_threshold = math.log2(skip_threshold) if skip_softmax else 0.0
